...
class EncoderDecoderAugv3Abrm(AttModel):
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        if len(state) == 0:
            ...
            # print('it type and state type', it.unsqueeze(1).type(), state[0][0].type())
            device = torch.device('cpu')
            state_cpu = state[0][0].to(device)
            it_cpu = it.unsqueeze(1).to(device)
            # print('it type and state type after changing', it.unsqueeze(1).type(), state_cpu.type())
            ys = torch.cat([state_cpu, it_cpu], dim=1)
